...
=====================================================================
Optimized for Isolated Anaconda Environment with PyTorch 1D-CNN Brain
"""
        ...
        while self.running:
            if not self.is_simulating and tlpax:
                try:
                    ...
                    logger.error(f"Polarimeter query failed: {e}")
            else:
    ...
    def _process_real_data(self, raw_data: str) -> None:
        try:
            ...
            if len(msg) >= 13:
                ...
                logger.warning(f"Polarimeter returned too few fields ({len(msg)}): {raw_data!r}")
        except ValueError as e:
            logger.error(f"Could not parse polarimeter response {raw_data!r}: {e}")
...
class PoincareDashboard(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # --- MLP MODEL LOADING ---
        try:
            self.mlp_model = PolarizationMLP(in_f=9)
            self.mlp_model.load_state_dict(
                torch.load("mlp_brain_weights.pt", map_location=torch.device('cpu')))
            self.mlp_model.eval()
            logger.info("Successfully loaded MLP brain (mlp_brain_weights.pt).")
        except Exception as e:
            logger.error(f"Failed to load MLP brain: {e}")
    ...
    def _init_ui(self) -> None:
        ...
        self.btn_ml_collect.clicked.connect(lambda: self.start_ml_data_collection(4000))
        side_layout.addWidget(self.btn_ml_collect)

        self.ram_label = QtWidgets.QLabel("RAM Usage: 0.0 MB")
        self.ram_label.setStyleSheet(f"color: {Config.COLOR_ACCENT}; font-size: 14px; font-weight: bold; padding-top: 10px;")
        side_layout.addWidget(self.ram_label)
    ...
    def _on_data_received(self, data: List[float]) -> None:
        ...
        raw_sop = np.array([s1, s2, s3], dtype=float)
        ...
        if self.logging_enabled:
            ...
            self.log_data.append([
                current_time, s1, s2, s3, dop, pwr, psi_deg, chi_deg, ram_usage_mb
            ])

        target_arr = np.array(self.current_target_vec)[:3].astype(float)
        dot_product = float(np.dot(norm_sop, target_arr))
        safe_dot = max(-1.0, min(1.0, dot_product)) 
        dist_to_target = float(math.degrees(math.acos(safe_dot)))
        ...
        # =======================================================================
        # --- MLP LIVE INFERENCE ---
        # MLP maps current 3 voltages directly to predicted Stokes vector.
        # No history buffer needed - each sample is independent.
        # =======================================================================
        try:
            if hasattr(self, 'mlp_model'):
                V = np.array(self.epc_voltages[:3], dtype=np.float32)
                # Same 9-feature transform used during training
                V_n = V / 10.0
                V_s = np.sin(np.pi * V_n)
                V_c = np.cos(np.pi * V_n)
                feat = np.concatenate([V_n, V_s, V_c]).astype(np.float32)

                with torch.no_grad():
                    inp = torch.tensor(feat).unsqueeze(0)          # (1, 9)
                    out = self.mlp_model(inp).numpy()[0]            # (3,)

                predicted_stokes = out / (np.linalg.norm(out) + 1e-9)
                self.live_predicted_s1 = float(predicted_stokes[0])
                self.live_predicted_s2 = float(predicted_stokes[1])
                self.live_predicted_s3 = float(predicted_stokes[2])
        except Exception as e:
    ...
    def _run_epc_optimization(self, current_error_deg):
        ...
        if self.ram_tracker.history['ML']:
            latest_mem = self.ram_tracker.history['ML'][-1][1]
            self.ram_label.setText(f"RAM Usage: {latest_mem:.1f} MB")
        
        now = time.time()
        TARGET_LOCK_ZONE = 5.0  
        WAKE_UP_THRESHOLD = 8.5 
        STABILITY_TIME = 1.0
        ...
        if not hasattr(self, 'bad_step_counter'): self.bad_step_counter = 0
        ...
        if current_error_deg >= 15.0:
            # PHASE 1: MLP-GUIDED MONTE CARLO SEARCH
            # Score 512 candidate voltage triplets through the MLP in one batch,
            # pick the one predicted closest to the target, apply it.
            if hasattr(self, 'mlp_model'):
                V_curr = np.array(self.epc_voltages[:3], dtype=np.float32)
                N = 512
                c1 = np.random.uniform(0.0, 10.0, (N // 2, 3))
                c2 = np.clip(np.random.normal(V_curr, 3.0, (N // 2, 3)), 0, 10)
                candidates = np.vstack([c1, c2]).astype(np.float32)

                # Build 9-feature inputs for all candidates
                Vn = candidates / 10.0
                Vs = np.sin(np.pi * Vn)
                Vc = np.cos(np.pi * Vn)
                feats = np.hstack([Vn, Vs, Vc]).astype(np.float32)  # (N, 9)

                T = T_unit.astype(np.float32)
                with torch.no_grad():
                    preds = self.mlp_model(
                        torch.tensor(feats)).numpy()                  # (N, 3)
                preds_n  = preds / (np.linalg.norm(preds, axis=1, keepdims=True) + 1e-9)
                cos_sims = (preds_n * T).sum(axis=1)
                best_idx = int(np.argmax(cos_sims))
                pred_err = float(np.degrees(np.arccos(np.clip(cos_sims[best_idx], -1, 1))))
                logger.info(f"MLP coarse jump → predicted error {pred_err:.1f}°")
                self._set_all_voltages(candidates[best_idx])
            else:
                self._set_all_voltages(np.random.uniform(0.0, 10.0, 3))
            self.bad_step_counter = 0 
            
        else:
            # PHASE 2: SMOOTH APPROACH (Hill-Climbing optimization)
            step_mag = 0.10 
            if current_error_deg > self.last_error_deg:
                self.bad_step_counter += 1
                self.epc_step_size = -self.epc_step_size
                if self.bad_step_counter >= 2:
                    self.active_epc_channel = (self.active_epc_channel + 1) % 3
                    self.bad_step_counter = 0 
                    logger.info(f"Saddle point hit. Switching to EPC Channel {self.active_epc_channel}")
            else:
                self.bad_step_counter = 0
            ...
            if hasattr(self, 'epc'):
                try:
                    ...
                    logger.error(f"EPC write failed: {e}")

        self.last_error_deg = current_error_deg 

    

    def start_ml_data_collection(self, num_samples=4000):
        # 1. FORCE DISABLE AI TO PREVENT VOLTAGE OVERRIDES
        self.auto_compensate = False
        if self.btn_auto.isChecked():
            self.btn_auto.setChecked(False)
            self.btn_auto.setText("Start Auto-Compensate")

        # --- SETTLING PARAMETERS (tuned from diagnostic_hysteresis.py results) ---
        # Diagnostic showed the EPC needs 1.68s-2.52s to converge after a voltage
        # jump. We poll every 200ms and require 3 consecutive readings within
        # ML_CONVERGENCE_DEG of each other before logging, with a hard cap so a
        # pathological jump can't stall collection forever.
        self.ML_CHECK_INTERVAL_MS = 200
        self.ML_CONVERGENCE_DEG = 1.0
        self.ML_STABLE_READS_REQUIRED = 3
        self.ML_MAX_WAIT_S = 4.0
        ...
        self.ml_timer.timeout.connect(self._ml_check_tick)
        self._ml_set_random_voltages()
        self.ml_timer.start(self.ML_CHECK_INTERVAL_MS)
    ...
    def _ml_set_random_voltages(self):
        ...
        # Reset convergence tracking for the new jump
        self._ml_jump_start_time = time.time()
        self._ml_prev_sop = None
        self._ml_stable_count = 0

    def _ml_check_tick(self):
        """Polls at ML_CHECK_INTERVAL_MS. Only logs a sample once the SOP has
        actually converged after a voltage jump (or the max-wait safety cap
        is hit), instead of logging on a fixed blind delay."""
        current = np.array(self.current_sop[:3], dtype=float)
        elapsed = time.time() - self._ml_jump_start_time

        if self._ml_prev_sop is not None:
            diff_deg = self._angular_diff_deg(current, self._ml_prev_sop)
            if diff_deg < self.ML_CONVERGENCE_DEG:
                self._ml_stable_count += 1
            else:
                self._ml_stable_count = 0
        self._ml_prev_sop = current

        converged = self._ml_stable_count >= self.ML_STABLE_READS_REQUIRED
        timed_out = elapsed >= self.ML_MAX_WAIT_S

        if not (converged or timed_out):
            return  # keep waiting, sample again next tick

        # Settled (or gave up waiting) - log this sample
        ...
        if self.ml_samples_collected % 50 == 0 or self.ml_samples_collected == 1:
            tag = "OK" if converged else "TIMEOUT"
            self.btn_ml_collect.setText(f"COLLECTING... {self.ml_samples_collected} / {self.ml_samples_total} [{tag}]")
            if not converged:
                logger.warning(f"Sample {self.ml_samples_collected}: hit {self.ML_MAX_WAIT_S}s wait cap without converging")
    ...
    def save_log_to_csv(self, filename="polarization_session_log.csv"):
        ...
        headers = ["Timestamp", "S1", "S2", "S3", "DOP_percent", "Power_mW", "Azimuth_deg", "Ellipticity_deg", "RAM_Usage_MB"]
        try:
            with open(filename, mode='w', newline='') as file:
                writer = csv.writer(file)
    ...
    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        if hasattr(self, 'ram_tracker'):
         print("Generating RAM usage graph...")
         self.ram_tracker.plot_usage()
