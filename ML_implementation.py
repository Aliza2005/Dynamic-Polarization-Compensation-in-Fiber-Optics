...
class PoincareDashboard(QtWidgets.QMainWindow):
    ...
    def _init_ui(self) -> None:
        ...
        # Inside PoincareDashboard._init_ui, near the Metrics Groups
    ...
    def _on_data_received(self, data: List[float]) -> None:
        ...
        if self.logging_enabled:
            ...
            # 1. CALCULATE RAM FIRST!
            import psutil, os
            process = psutil.Process(os.getpid())
            ram_usage_mb = process.memory_info().rss / (1024 * 1024)

            # 2. NOW APPEND IT
            # Order: Timestamp, S1, S2, S3, DOP_%, Power_mW, Azimuth_deg, Ellipticity_deg, RAM_MB
            self.log_data.append([
                current_time, s1, s2, s3, dop, pwr, psi_deg, chi_deg, ram_usage_mb
            ])

        # --- THE MATH ARMOR ---
        # 1. Force the target vector into a strict 3-element numpy float array
        target_arr = np.array(self.current_target_vec)[:3].astype(float)
        
        # 2. Calculate pure Python dot product
        dot_product = float(np.dot(norm_sop, target_arr))
        
        # 3. Pure Python clamp (Indestructible against math domain errors)
    ...
    def _run_epc_optimization(self, current_error_deg):
        """
        STABLE WIDE-LOCK VERSION:
        Added Counter Logic and Bad-Step verification to prevent INVALID_HANDLE errors.
        """
        import time
        import numpy as np
        ...
        # NEW: Counter logic initialization
        ...
        # --- 4. HARDWARE THROTTLE (CRITICAL FOR STABILITY) ---
        # We keep the 250ms delay to ensure the FTDI buffer doesn't overflow
        ...
        if current_error_deg >= 15.0:
            ...
            # Final Safety: Only write if the handle is still present
            if hasattr(self, 'epc'):
                try:
    ...
    def save_log_to_csv(self, filename="polarization_session_log.csv"):
        ...
        # --- ADDED RAM HEADER HERE ---
        headers = [
            "Timestamp", "S1", "S2", "S3", "DOP_percent", 
            "Power_mW", "Azimuth_deg", "Ellipticity_deg", "RAM_Usage_MB"
        ]
    ...
    # Generate the RAM usage plot on exit
        if hasattr(self, 'ram_tracker'):
         print("Generating RAM usage graph...")
         self.ram_tracker.plot_usage()
    
        self.save_log_to_csv() # Existing log save
