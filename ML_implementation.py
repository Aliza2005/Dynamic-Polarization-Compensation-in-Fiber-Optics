...
class PoincareDashboard(QtWidgets.QMainWindow):
    ...
    def _init_ui(self) -> None:
        ...
        self.btn_ml_collect.clicked.connect(lambda: self.start_ml_data_collection(5000))
    ...
    def _run_epc_optimization(self, current_error_deg):
        """FINAL OPTIMIZED: High-Speed AI Search & Rapid Disturbance Rejection"""
        import time
        import numpy as np
        ...
        def _live_error():
            S = np.array(self.current_sop[:3], dtype=float)
            T = np.array(self.current_target_vec[:3], dtype=float)
            sn, tn = np.linalg.norm(S), np.linalg.norm(T)
            if sn == 0 or tn == 0:
                return None, None, None
            Su, Tu = S / sn, T / tn
            err = np.degrees(np.arccos(np.clip(np.dot(Su, Tu), -1.0, 1.0)))
            return err, Su, Tu

        # ── Gate 0: initial read ───────────────────────────────────────────────
        current_error_deg, S_unit, T_unit = _live_error()
        if current_error_deg is None:
            return

        # ══════════════════════════════════════════════════════════════════════
        # SPEED OPTIMIZATION 1: DYNAMIC DELAY
        # Phase 1 needs a bit more time for the AI. Phase 2 can fire rapidly.
        # ══════════════════════════════════════════════════════════════════════
        is_phase2 = getattr(self, 'in_phase2', False)
        target_delay = 0.05 if is_phase2 else 0.15  # 20Hz in Phase 2, ~6Hz in Phase 1

        if now - getattr(self, 'last_epc_update_time', 0) < target_delay:
            return
        self.last_epc_update_time = now

        # ── Gate 1: re-read after delay ────────────────────────────────────────
        current_error_deg, S_unit, T_unit = _live_error()
        if current_error_deg is None:
            return
        ...
        if not hasattr(self, 'epc_bad_steps'): self.epc_bad_steps = 0
        ...
        # ══════════════════════════════════════════════════════════════════════
        # PRECISION HYSTERESIS (LOCKING)
        # ══════════════════════════════════════════════════════════════════════
        if not hasattr(self, 'is_locked'):
            self.is_locked = False

        if current_error_deg < 2.0:
            self.is_locked = True
            self.epc_step_size = 0.05  # Reset step size for tiny corrections later
        elif current_error_deg > 5.0:
            self.is_locked = False

        if self.is_locked:
            self.btn_auto.setText(f"✓ LOCKED ON TARGET  ({current_error_deg:.2f}°)")
        ...
        # ══════════════════════════════════════════════════════════════════════
        # PHASE BOUNDARY HYSTERESIS
        # ══════════════════════════════════════════════════════════════════════
        if current_error_deg < 15.0:
            if not self.in_phase2:
                self.last_error_deg = current_error_deg
            self.in_phase2 = True
        elif current_error_deg > 20.0:
            self.in_phase2 = False

        # ══════════════════════════════════════════════════════════════════════
        # PHASE 1: MACRO SEARCH (AI) - Optimized for speed
        # ══════════════════════════════════════════════════════════════════════
        if not self.in_phase2:
            self.btn_auto.setText(f"PHASE 1: MACRO SEARCH  ({current_error_deg:.1f}°)")
            self.btn_auto.setStyleSheet("background-color: #9C27B0; color: white; font-weight: bold; padding: 12px; border-radius: 8px;")

            if getattr(self, 'epc_brain', None) is None or getattr(self, 'epc_scaler', None) is None:
                return

            V_current_arr = np.array(self.epc_voltages[:3], dtype=float)
            S_current_r = S_unit.reshape(1, 3)
            V_current = V_current_arr.reshape(1, 3)

            # SPEED OPTIMIZATION 2: Lower sample count (10x faster computation)
            n_local = 400
            n_global = 100
            total_n = n_local + n_global
            
            local_v = np.random.normal(V_current_arr, 1.0, (n_local, 3))
            global_v = np.random.uniform(0.0, 10.0, (n_global, 3))
            test_v = np.clip(np.vstack([local_v, global_v]), 0.0, 10.0)
            
            X_macro = np.hstack([np.repeat(S_current_r, total_n, axis=0), np.repeat(V_current, total_n, axis=0), test_v])
            dS = self.epc_brain.predict(self.epc_scaler.transform(X_macro))
            pred = np.repeat(S_current_r, total_n, axis=0) + dS
            
            norms = np.linalg.norm(pred, axis=1, keepdims=True)
            pred /= np.where(norms == 0, 1.0, norms)

            dots = np.sum(pred * T_unit, axis=1)
            best_5 = np.argpartition(-dots, 5)[:5]
            target_v = np.mean(test_v[best_5], axis=0)

            step_vector = target_v - V_current_arr
            if not hasattr(self, '_macro_momentum'):
                self._macro_momentum = step_vector
            else:
                self._macro_momentum = (0.6 * step_vector) + (0.4 * self._macro_momentum)

            # SPEED OPTIMIZATION 3: Increased max jump from 1.0 to 1.5 to cover ground faster
            new_v = np.clip(V_current_arr + np.clip(self._macro_momentum, -1.5, 1.5), 0.0, 10.0)
            self._set_all_voltages(new_v.tolist())
            
            self.last_error_deg = current_error_deg
            return

        # ══════════════════════════════════════════════════════════════════════
        # PHASE 2: ACTIVE 3-WAVEPLATE DISTURBANCE REJECTION - High Speed
        # ══════════════════════════════════════════════════════════════════════
        self.btn_auto.setText(f"PHASE 2: FIGHTING DRIFT/STRESS... ({current_error_deg:.1f}°)")
        self.btn_auto.setStyleSheet("background-color: #00C853; color: white; font-weight: bold; padding: 12px; border-radius: 8px;")

        # SPEED OPTIMIZATION 4: More aggressive stepping to close the gap rapidly
        if current_error_deg > 10.0:
            base_step = 0.30
        elif current_error_deg > 5.0:
            base_step = 0.15
        else:
